...
func (transfer *Transfer) ReadPkg() (msg message.Message, err error) {
	...
	fmt.Println("等待读取...")
	if err != nil {
		fmt.Printf("conn.Read(buf[:4]) err, err=%v\n", err)
		return
	}
	fmt.Printf("读到的buf=%v\n", transfer.Buf[:4])
	// 根据读到的长度读取数据
	...
	if uint32(n) != pkgLength || err != nil {
		fmt.Printf("conn.Read(buf[:pkgLength]) fail, err=%v\n", err)
		return
	}
	err = json.Unmarshal(transfer.Buf[:pkgLength], &msg)
	if err != nil {
		fmt.Printf("json.Unmarshal(buf[:pkgLength], &msg) fail, err=%v\n", err)
		return
	}
	return
}
...
func (transfer *Transfer) WritePkg(data []byte) (err error) {
	...
	if n != 4 || err != nil {
		fmt.Printf("conn.Write(buf) err, err=%v\n", err)
		return
	}
	// 发送数据
	n, err = transfer.Conn.Write(data)
	if n != int(pkgLength) || err != nil {
		fmt.Printf("conn.Write(data) err, err=%v\n", err)
		return
	}
	return
}
